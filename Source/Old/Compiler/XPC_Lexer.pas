...
Type
  CustomLexer = Class(StateMachine)
    ...
    yystate: integer; // current state of lexical analyzer
      ...
      _Streams:Array[0..MaxStreams] Of Stream;
      _StreamIndex:Integer;
      ...
      Procedure yypushStream(S:Stream);
    ...
    procedure returnc(c: AnsiChar);
      ...
      Function yymoreStreams():Boolean;

      Function GetCurrentSource():Stream;
  ...
  Public
    ...
    Property CurrentSource:Stream Read GetCurrentSource;
  End;
...
Var
  I:Integer;
Begin
  If (_bufptr = 0) And (Not CurrentSource.EOF) Then
  Begin
    CurrentSource.ReadLine(yyline);
    Inc(yylineno);
    yycolno := 1;
    _Buf[1]  := nl;
...
Function CustomLexer.GetCurrentSource():Stream;
Begin
  If (_StreamIndex>=0) Then
    Result := _Streams[_StreamIndex]
  Else
    Result := Nil;
End;
...
Function CustomLexer.yymoreStreams: Boolean;
Begin
  Result := _StreamIndex>0;
End;

Procedure CustomLexer.yypushStream(S: Stream);
Begin
  Inc(_StreamIndex);
  _Streams[_StreamIndex] := S;
End;
